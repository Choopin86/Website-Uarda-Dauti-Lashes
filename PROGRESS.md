...
- **Services page Section 1 "Page Header"** — new `businessIdentity.json` fields `servicesIntroText` (drafted intro line, al/en) and `servicesCoverImage` (media ref); real cover photo now added at `public/media/pictures/` with `media.json`'s `services-cover-image-id` entry pointing to it. New `transformServicesIntro`/`renderServicesIntro` pair renders a full-bleed dark banner matching the Home hero formula — cover image fills the section, a radial + linear dark scrim overlay plus text-shadow keep the intro line readable against the photo, text centered horizontally and vertically in light, bold serif type. No links/buttons per the doc's constraint. Wired into `services.js` and a new `.services-intro` placeholder in `services.html`, styled via `src/styles/sections/services-intro.css`. Verified in-browser (desktop + mobile, both languages, no console errors) including the contrast pass.
